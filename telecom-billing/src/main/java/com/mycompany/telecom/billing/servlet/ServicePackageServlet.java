...
public class ServicePackageServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest req, HttpServletResponse resp)
        ...
        try {
            if (path == null || path.equals("/")) {
                ...
                renderForm(req, resp, new ServicePackage(), false);
            } else if (path.startsWith("/edit/")) {
                int id = Integer.parseInt(path.substring(6));
                ServicePackage sp = dao.findById(id);
                if (sp == null) {
                    resp.sendError(404);
                    return;
                }
    ...
    // ── FORM ─────────────────────────────────────────────────────────────────
    private void renderForm(HttpServletRequest req, HttpServletResponse resp, ServicePackage sp, boolean editing)
            throws Exception {
        String ctx = req.getContextPath();
        ...
        out.print(HtmlLayout.breadcrumb(
                ...
                "Service Packages", ctx + "/packages/",
                title, null));

        String voiceSel = "voice".equals(sp.getType()) ? " selected" : "";
        String dataSel = "data".equals(sp.getType()) ? " selected" : "";
        String smsSel = "sms".equals(sp.getType()) ? " selected" : "";
        ...
        out.printf("""
              ...
              <div class='card-body'>
                <form method='post' action='%s'>
                  <div class='form-grid'>
                    ...
                    <div class='form-group'>
                      <label>Amount (minutes / MB / messages)</label>
                      <input type='number' step='0.0001' name='amount' value='%s' placeholder='e.g. 1000' min='0'>
                    </div>
                    <div class='form-group'>
                      <label>Priority (lower = consumed first)</label>
                      <input type='number' name='priority' value='%s' placeholder='1' min='1' max='99'>
                    </div>
                  </div>
            ...
            """,
                ...
                HtmlLayout.e(sp.getName()),
                voiceSel, dataSel, smsSel,
